...
function LoginInner({ adminHost }: { adminHost: boolean }) {
  const { signIn } = useAuth();
  const router = useRouter();
  const params = useSearchParams();
  const returnTo = params.get("return_to") || (adminHost ? "/admin" : "/requests");

  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPw, setShowPw] = useState(false);
  // 운영자 로그인에는 약관 동의를 묻지 않는다 — 이미 가입된 내부 계정이다.
  const [agreed, setAgreed] = useState(adminHost);
      ...
      <div className="w-full max-w-[420px] rounded-2xl border border-border bg-card p-8">
        <div className="mb-6 text-center">
          <h1 className="mb-1 text-2xl font-extrabold text-foreground">
            {adminHost ? "운영자 로그인" : "로그인"}
          </h1>
          <p className="text-sm text-text-secondary">
            {adminHost ? "우주딜러 운영 콘솔" : "B2B 폐PC 업사이클링 플랫폼"}
          </p>
        </div>
        ...
        {/* 소셜 로그인·회원가입은 고객 흐름이다. 운영자 계정은 가입으로 만들지 않는다. */}
        {!adminHost && (
          <>
...
export function LoginForm({ adminHost }: { adminHost: boolean }) {
  return (
    <Suspense fallback={null}>
      <LoginInner adminHost={adminHost} />
    </Suspense>
  );
}

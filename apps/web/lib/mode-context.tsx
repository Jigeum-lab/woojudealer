 ...
 *   buy  — 되살린 PC를 사려는 사람 (견적·구성) · 기본
 *   sell — 폐PC를 처분하려는 기업 담당자 (수거·삭제·증명)
 *
 * 플랫폼의 기본은 구매다. 처분은 헤더 오른쪽 링크로 들어간다.
 * 헤더와 랜딩 본문이 같은 값을 봐야 해서 컨텍스트로 올리고,
...
export function ModeProvider({ children }: { children: React.ReactNode }) {
  // 서버 렌더와 첫 클라이언트 렌더는 항상 기본값(buy)으로 맞춘다.
  // 저장값을 초기값으로 쓰면 두 결과가 달라져 hydration이 어긋난다.
  const [mode, setModeState] = useState<SiteMode>("buy");
  const [hydrating, setHydrating] = useState(true);

  useEffect(() => {
    try {
      // ?mode= 링크가 저장값보다 우선한다 — 보낸 사람 의도가 먼저다
